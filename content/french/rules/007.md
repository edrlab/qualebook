...
Meo: 
    - "Dans le fichier `nav.xhtml`, créer une section de navigation spécifique pour les points de repère (landmarks) en utilisant la balise `<nav>` avec l'attribut `epub:type` `landmarks`. À l'intérieur de la balise `<nav>`, lister les liens vers les différentes sections clés du livre. Chaque lien (`<a>`) doit avoir un attribut `epub:type` qui identifie sémantiquement la section de destination. Les epub:type pertinents sont `glossary`, `index`, `bibliography`."
