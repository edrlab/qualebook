---
title: "Les conditions d'exploitation et de fouille des données du livre sont indiquées" 
abstract: "Les pratiques de fouille de textes et de données (TDM) sont utilisées par des entités privées et publiques pour analyser de grandes quantités de données, y compris des contenus protégés par le droit d'auteur comme les livres numériques. L'utilisation de ces techniques peut être soumise à différentes conditions juridiques, et leur exploitation en masse nécessite de pouvoir identifier rapidement les œuvres pour lesquelles les techniques de TDM ne sont pas autorisées par les ayants droits."
categories: 
    - "gestion des droits"
opquast: 'N/A'
indiceebook: '002'
description: "Règle n°02"
before: "001"
weight: "2"
after: "003"
actif: '1'
layout: rules
date: 2025-01-28
objectif: 
    - "Signaler aux systèmes d'indexation si l'exploitation des données du livre par des techniques de TDM est autorisée ou interdite."
Meo: 
    - "Pour un EPUB3 pour lequel il est souhaité de réserver les droits de TDM, ajouter au fichier OPF la métadonnée `tdm:reservation`. En présence d'une licence consultable, il est possible d'en donner l'adresse avec la métadonnée `tdm:policy`."
    - "D'autres techniques existent pour les fichiers EPUB2 et PDF. Des exemples de mise en oeuvre sont disponibles sur la recommandation [TDM Reservation Protocol (TDMRep), en anglais](https://www.w3.org/community/reports/tdmrep/CG-FINAL-tdmrep-20240510/)."
    - "Pour lever toute ambiguïté et faire connaître la politique aux humains autant qu'aux machines, il est conseillé d'ajouter à la page de copyright une mention écrite de la politique de la maison d'édition en matière de TDM."
Controle: 
    - "Vérifier la présence des métadonnées `tdm:reservation` et `tdm:policy` ainsi que les mentions correspondantes sur la page de copyright."
epubcheck: false
ace: false
humancheck: true
ReadiumGoToolkit: 
Source: 
    - "SNE"
Referentiel: 
    - "[TDM Reservation Protocol (TDMRep), en anglais](https://www.w3.org/community/reports/tdmrep/CG-FINAL-tdmrep-20240510/)"
steps: 
    - "Production numérique"
    - "Projet éditorial"
tags: 
    - "Juridique"
---
